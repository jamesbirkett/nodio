...
function Nodio (credentials) {
	...
	nodio.filterItems = function (request, callback) {
		validateCredentials(function (err) {
			if (!err) {
				// Make a post request to filter items from the current app, passing the access token as a header
				rest.postJson('https://api.podio.com/item/app/' + credentials.app_id + '/filter/', request, {
					headers: {'Authorization': 'OAuth2 ' + access_token}
				})
					.on('complete', function(data, response) {
						// If everything's ok
	...
	nodio.getCommentsOnItem = function (item_id, callback) {
		validateCredentials(function (err) {
			if (!err) {
				// Make a get request to retrieve all comments on the specified item
				rest.get('https://api.podio.com/comment/item/' + item_id, {
					headers: {'Authorization': 'OAuth2 ' + access_token}
				})
					.on('complete', function(data, response) {
						// If everything's ok
	...
	nodio.addCommentToItem = function (item_id, comment, callback) {
		validateCredentials(function (err) {
			if (!err) {
				// Make a post request to add a comment to the specified item
				rest.postJson('https://api.podio.com/comment/item/' + item_id, comment, {
					headers: {'Authorization': 'OAuth2 ' + access_token}
				})
					.on('complete', function(data, response) {
						// If everything's ok
